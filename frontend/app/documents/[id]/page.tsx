  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Document Intelligence Workspace</h1>
          <p className="text-xs font-mono text-slate-500">ID: {documentId}</p>
        </div>
        <span className="px-3 py-1 bg-purple-50 text-purple-900 border border-purple-300 rounded font-bold text-xs">
          AI PROPOSAL — ADVISORY ONLY
        </span>
      </div>

      {isLoading ? (
        <div className="p-8 text-center text-slate-500 text-xs">Loading document intelligence data...</div>
      ) : (
        <>
          {docStatus && (
            <div className="bg-white p-6 rounded-lg border border-slate-200 shadow-sm space-y-4">
              <h2 className="text-sm font-bold text-slate-800">Document Metadata & Security</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                <div>
                  <div className="text-slate-500 text-[10px] uppercase font-bold">Filename</div>
                  <div className="font-semibold text-slate-900">{docStatus.original_filename}</div>
                </div>
                <div>
                  <div className="text-slate-500 text-[10px] uppercase font-bold">Classification</div>
                ...
                <div>
                  <div className="text-slate-500 text-[10px] uppercase font-bold">Quarantine</div>
                  <StatusBadge status={docStatus.quarantine_status} />
                </div>
                <div>
                  <div className="text-slate-500 text-[10px] uppercase font-bold">Malware Scan</div>
